        ...
        if received_message:
            try:
                ...
                if command == "pong" and sender_id == cube_id:
                    logger.info(
                        f"Success! Received pong from {sender_id}."
                    )
                else:
                    logger.warning(
...
def send_leaderboard_power_loop(logger, uhf_packet_manager, sleep_helper, cube_ids=DEFAULT_CUBE_IDS):
    """
